...
2) ✅ single fasta 不再 half-label（解析不到标签直接报错，避免伪造标签导致 1.0）
...
DATASETS_CONFIG = {
    "E.coli": {
        # single 文件：header 中应包含 |1 或 |0 或 label=1/0 等
        "single": os.path.join(DATA_ROOT_DIR, "E.coli", "data.fasta")
    }
}
...
def read_fasta_raw_strict(
    ...
    - default_label == None：从 header 解析标签
        - allow_missing_labels=False：只要有解析不到的标签，直接报错（推荐）
        - allow_missing_labels=True：允许缺失，用 missing_label_fill 兜底（不推荐）

    ✅ 不再使用 half-label 规则（避免伪造标签导致指标虚高）
    """
...
def prepare_dataset(name, config):
    ...
    if "pos" in config and "neg" in config:
        ...
        ph, ps, pl = read_fasta_raw_strict(config['pos'], default_label=1)
        print(f"    [Load] 负样本: {os.path.basename(config['neg'])}")
        nh, ns, nl = read_fasta_raw_strict(config['neg'], default_label=0)
        headers = ph + nh
        seqs = ps + ns
        labels = pl + nl
        ...
        headers, seqs, labels = read_fasta_raw_strict(
            config["single"],
            default_label=None,
            allow_missing_labels=False  # ✅ 强烈推荐：缺标签直接报错停止
        )
    ...
    if pos_cnt == 0 or neg_cnt == 0:
        print(f"[ERROR] {name} 数据缺失（标签未正确解析成0/1或序列为空）。")
        return [], [], []
...
def sanitize_ilearn_csv_safe(in_csv, out_csv):
    ...
    更安全的 iLearn CSV 清洗：
    - 删除样本ID列（常见为第一列）
    - 删除 label/class/y 等标签列（可能在最后，不一定是第一列）
    """
...
def generate_ilearn_features(data_dir, headers, seqs, labels, ilearn_basic, ilearn_pse):
    """
    生成 iLearn 特征，并用“安全清洗”确保 label 不会混进特征
    """
    ...
    for method, script, name in tasks:
        ...
        # ✅ 安全清洗：删除 label/class/y 列 + 可能的样本ID列
...
def generate_w2v_txt(seqs, idx_list, w2v_mat, out_path, k=W2V_KMER, out_len=W2V_OUT_LEN):
    """
    将序列编码为固定长度 out_len 的 Word2Vec 特征（仅编码，不训练，不泄露）
    """
