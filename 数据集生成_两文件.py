...
DATASETS_CONFIG = {
    "B.subtilis": {
        "pos": "/home/user012/experments/Desktop/pythonProjectexperments/iPro-WAEL-main/iPro-WAEL-main/data/B.subtilis/Bacillus_prom.fasta",
        "neg": "/home/user012/experments/Desktop/pythonProjectexperments/iPro-WAEL-main/iPro-WAEL-main/data/B.subtilis/Bacillus_non_prom.fasta",
    }
}
...
def prepare_dataset(name, config):
    ...
    # ✅ 用 pos/neg 两个 .fa 文件（强制标签）
    if "pos" in config and "neg" in config:
        print(f"    [Load] 正样本(pos): {os.path.basename(config['pos'])}")
        ph, ps, pl = read_fasta_raw_strict(config["pos"], default_label=1, allow_missing_labels=True)

        print(f"    [Load] 负样本(neg): {os.path.basename(config['neg'])}")
        nh, ns, nl = read_fasta_raw_strict(config["neg"], default_label=0, allow_missing_labels=True)
