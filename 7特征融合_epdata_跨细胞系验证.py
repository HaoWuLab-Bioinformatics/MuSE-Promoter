...
# 输出文件名
OUTPUT_FILE = "Cross_Cell_Validation_Results.xlsx"
...
class EarlyStopping:
    """
    当验证集损失在 patience 轮次内没有下降时，停止训练并保存最佳模型。
    """
    def __init__(self, patience=7, verbose=False, delta=0, path='checkpoint.pt'):
        """
        Args:
            patience (int): 损失没变好的容忍轮数
            verbose (bool): 是否打印日志
...
def run_hybrid_training(X_seq_tr, X_feat_tr, y_tr, X_seq_w, X_feat_w, y_w, X_seq_te, X_feat_te):
    ...
    # 4. 训练结束后，加载最优模型 (Load Best State)
...
def load_data_for_cell(cell_name, sub_folder="train"):
    """
    通用数据加载函数
    sub_folder: 通常训练集用 'train', 测试集用 'test'
    """
...
def save_result_realtime(result_dict, filename):
    """
    接收一行结果字典，立即追加写入 CSV 文件
    """
    df = pd.DataFrame([result_dict])

    # 如果文件不存在，写入表头；如果存在，不写表头，直接追加 (mode='a')
...
def main():
    ...
    # 改为 .csv 格式，方便实时追加
    REALTIME_FILE = "Cross_Cell_Validation_Realtime.csv"
    ...
    for train_cell in CELL_LINES:
        ...
        for test_cell in CELL_LINES:
            ...
            # --- Model 2: Hybrid Transformer ---
            print("  Running Hybrid-Trans with Early Stopping...")

            # 【注意】这里新增了 parameter: y_w=y_w
            cnn_weight_proba, cnn_test_proba, cnn_test_class = run_hybrid_training(
                X_seq_tr=x_seq_tr, X_feat_tr=x_rf_tr, y_tr=y_tr,
                X_seq_w=x_seq_w, X_feat_w=x_rf_w, y_w=y_w,  # <--- 新增这行
            ...
            print(f"  [Saved] Results for {train_cell}->{test_cell} saved to csv.")

    print(f"\nAll Done. You can open {REALTIME_FILE} in Excel directly.")
